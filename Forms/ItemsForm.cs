    ...
    public partial class ItemsForm : Form
    {
                ...
                if (itemListView.SelectedItems.Count > 0)
                {
                    ...
                    if (returnedItem != null)
                    {
                        if (_itemManager.Update(item))
                        {
                            ...
                            {
                                purchaseDetails.Price = returnedItem.Price;
                                purchaseDetails.Name = returnedItem.Name;
                                purchaseDetails.PurchaseItem.SubTotal = returnedItem.Price * purchaseDetails.PurchaseItem.Quantity;
                            }
                ...
                if (itemListView.CheckedItems.Count > 0)
                {
                    using (TransactionScope scope = new TransactionScope())
                    {
                        foreach (ListViewItem listViewItem in itemListView.CheckedItems)
                        {
                            ...
                            PurchaseItem purchaseItem = new PurchaseItem(PrimaryId.GetGeneratedID(), _purchase.Id, item.Id, 1, item.Price);
                            item.Stocks -= 1;

                            if (_purchaseItemManager.Add(purchaseItem) && _itemManager.Update(item))
                            {
        ...
        private void AddItemButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (returnedItem != null)
                {
                    if (_itemManager.Add(returnedItem))
                    {
                        LoadData();
                    }
                    else
        ...
        private void SearchButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(searchName))
                {
                    ...
                    foreach (Item item in _itemManager.GetAll())
                    {
                        if (item.Name.Contains(searchName, StringComparison.OrdinalIgnoreCase))
                        {
                            ListViewItem listViewItem = new ListViewItem(new[] {string.Empty,
                                                                        item.Id.ToString(),
                                                                        item.Name,
